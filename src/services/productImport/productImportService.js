...
const COLUMN_ALIASES = {
  ...
  description: ['description', 'details', 'product description', 'rich description'],
  mainImageUrl: ['main image url', 'main image', 'image url', 'primary image', 'featured image'],
  ...
  colors: ['colors', 'available colors', 'colour', 'colours'],
  variants: ['variants', 'types', 'options'],
...
function splitMultiValue(value) {
  ...
  return normalizeStringArray(trimmed.split(/\r?\n|;/));
...
function buildPreparedRow(row, index, headerMap, context) {
  ...
  const normalizedRow = {
    ...
    show_colors: colors.length > 0,
    available_colors: colors,
    show_type: variants.length > 0,
    available_types: variants,
    show_wattage: !!trimOrEmpty(getCellValue(row, headerMap, 'power')),
