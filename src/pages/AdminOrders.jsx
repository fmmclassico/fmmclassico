...
function getNextStatus(order) {
  ...
  if (s === 'shipped') {
    if (needsPaymentConfirmation) {
      return { newStatus: 'out_for_delivery', label: 'Mark Payment Confirmed', message: 'Product payment confirmed on delivery.' };
    }
...
export default function AdminOrders() {
  ...
  var updateDeliveryDateMutation = useMutation({
    ...
    onSuccess: function() { queryClient.invalidateQueries({ queryKey: ['adminOrders'] }); toast.success('Delivery date updated!'); }
  });
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1">
            ...
            {(method === 'deposit_balance' || method === 'pay_on_delivery') && order.balance_due > 0 && (
              <p className="text-xs font-bold text-orange-700 mt-2 bg-orange-50 px-2 py-1.5 rounded-lg inline-block">
                ₵{order.balance_due?.toFixed(2)} left to be paid on delivery
              </p>
            )}
            ...
            <div className="mt-3 pt-2 border-t border-gray-100">
              ...
              {!isClosed && (
                <div className="flex items-center gap-2 mt-1.5">
                  <Input type="date" className="text-xs h-8 w-40" value={deliveryDates[order.id] || order.estimated_delivery || ''} onChange={function(e) { setDeliveryDates(function(p) { return { ...p, [order.id]: e.target.value }; }); }} />
                  <Button size="sm" variant="outline" className="text-xs h-8" onClick={function() { var d = deliveryDates[order.id]; if (d) updateDeliveryDateMutation.mutate({ order: order, date: d }); else toast.error('Pick a date first'); }} disabled={updateDeliveryDateMutation.isPending}>
                    Set
                  </Button>
                </div>
              )}
            ...
            {/* Action buttons - HIDDEN after delivered/cancelled */}
            {!isClosed && (
              <div className="flex flex-wrap gap-2 mt-3 pt-2 border-t border-gray-100">
                ...
                {next && order.payment_status === 'paid' && (
                  <Button size="sm" className="text-xs h-8 bg-blue-600 hover:bg-blue-700" onClick={function() { updateStatusMutation.mutate({ order: order, newStatus: next.newStatus, message: next.message }); }} disabled={updateStatusMutation.isPending}>
                    {next.label}
                  </Button>
                )}
                {next && order.payment_status !== 'paid' && (
                ...
                <Button size="sm" variant="destructive" className="text-xs h-8" onClick={function() { updateStatusMutation.mutate({ order: order, newStatus: 'cancelled', message: 'Cancelled by admin.' }); }} disabled={updateStatusMutation.isPending}>
                  Cancel
                </Button>
              </div>
            )}
