...
export default function AdminOrders() {
  ...
  var sendAdminMessageMutation = useMutation({
    mutationFn: async function({ order, message }) {
      await Promise.all([
        ...
        base44.integrations.Core.SendEmail({ to: order.customer_email, from_name: 'FMM CLASSICO', subject: 'Message about your order #' + order.order_number, body: 'Hi ' + order.customer_name + ',' + NL + NL + message + NL + NL + 'For help: 0208207543' + NL + NL + 'FMM CLASSICO Team' })
      ]);
    },
    onSuccess: function(_, variables) { setAdminMessages(function(prev) { return { ...prev, [variables.order.id]: '' }; }); toast.success('Message sent!'); }
  });

  var updateStatusMutation = useMutation({
    mutationFn: async function({ order, newStatus, message }) {
      var newTrackingUpdates = (order.tracking_updates || []).concat([{ status: statusConfig[newStatus]?.label || newStatus, message: message, timestamp: new Date().toISOString() }]);
      ...
      var notifMap = {
        ...
        out_for_delivery: { title: 'Payment Confirmed', msg: 'Product payment for order #' + order.order_number + ' confirmed. Thank you!', type: 'payment_confirmed' },
      ...
      if (notif) {
        var emailBody = 'Hi ' + order.customer_name + ',' + NL + NL + notif.msg + NL + NL + 'Order: #' + order.order_number + NL + 'Total: GHS ' + (order.total_amount || 0).toFixed(2) + NL + 'Delivery: ' + (order.delivery_address || 'N/A') + NL + NL + 'FMM CLASSICO' + NL + '0208207543';
        await Promise.all([
          ...
          base44.integrations.Core.SendEmail({ to: order.customer_email, from_name: 'FMM CLASSICO', subject: notif.title + ' - Order #' + order.order_number, body: emailBody })
        ]);
      }
    },
    onSuccess: function() { queryClient.invalidateQueries({ queryKey: ['adminOrders'] }); toast.success('Order updated! Customer notified.'); }
  });
  ...
  var handleToggleSelect = function(orderId) { setSelectedOrders(function(prev) { return prev.includes(orderId) ? prev.filter(function(id) { return id !== orderId; }) : prev.concat([orderId]); }); };
  var handleSelectAll = function(orderList) {
    var ids = orderList.map(function(o) { return o.id; });
    var allSelected = ids.every(function(id) { return selectedOrders.includes(id); });
    if (allSelected) { setSelectedOrders(function(prev) { return prev.filter(function(id) { return !ids.includes(id); }); }); }
    else { setSelectedOrders(function(prev) { return [...new Set(prev.concat(ids))]; }); }
  };
  ...
  var renderOrderCard = function(order) {
    ...
    var isClosed = ['delivered', 'cancelled', 'returned'].includes(order.status);
          ...
          <div className="flex-1">
            ...
            {(method === 'deposit_balance' || method === 'pay_on_delivery') && order.balance_due > 0 && (
              <p className="text-xs font-semibold text-orange-700 mt-1.5 bg-orange-50 px-2 py-1 rounded inline-block">
                Balance due on delivery: ₵{order.balance_due?.toFixed(2)}
              </p>
            )}
            ...
            {!isClosed && (
              <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-gray-100">
                ...
                {next && order.payment_status === 'paid' && (
                  <Button size="sm" className="text-xs bg-blue-600 hover:bg-blue-700" onClick={function() { updateStatusMutation.mutate({ order: order, newStatus: next.newStatus, message: next.message }); }} disabled={updateStatusMutation.isPending}>
                    {next.label}
                  </Button>
                )}
                {next && order.payment_status !== 'paid' && (
                ...
                <Button size="sm" variant="destructive" className="text-xs" onClick={function() { updateStatusMutation.mutate({ order: order, newStatus: 'cancelled', message: 'Cancelled by admin.' }); }} disabled={updateStatusMutation.isPending}>
                  Cancel
                </Button>
              </div>
            )}
            ...
            {!isClosed && (
              <div className="mt-3 flex gap-2">
                <Textarea className="text-xs flex-1" rows={1} placeholder="Message to customer..." value={adminMessages[order.id] || ''} onChange={function(e) { setAdminMessages(function(prev) { return { ...prev, [order.id]: e.target.value }; }); }} />
                <Button size="sm" variant="outline" onClick={function() { sendAdminMessageMutation.mutate({ order: order, message: adminMessages[order.id] }); }}><Send className="h-3 w-3" /></Button>
              </div>
            )}
          </div>
        </div>
