...
function formatVariantSummary(item) { if (item?.variant_summary) return item.variant_summary; const parts = []; if (item?.selected_color) parts.push(`Color: ${item.selected_color}`); if (item?.selected_wattage) parts.push(`Wattage: ${item.selected_wattage}`); if (item?.selected_type) parts.push(`Type: ${item.selected_type}`); return parts.join(' â€¢ '); }
...
export default function OrderTracking() {
  ...
  const steps = !isTwoStageOrder(order) ? [
    ...
    { label: 'Balance Paid', desc: `Outstanding amount of â‚µ${balanceDue.toFixed(2)} has been received.`, done: remainingPaid },
    { label: 'Product Successfully Delivered', desc: 'Product is handed over only after the balance payment is complete.', done: rank >= 5 },
  ];

  const payLabel = !isTwoStageOrder(order) ? (initialPaid ? 'Fully Paid' : 'Pending Payment') : remainingPaid ? 'Fully Paid' : initialPaid ? `Initial Payment Received, â‚µ${balanceDue.toFixed(2)} left` : 'Pending Initial Payment';
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-8">
      <div className="max-w-2xl mx-auto px-4 pt-6">
        ...
        <Card className="p-5 bg-white mb-4"><h2 className="text-sm font-bold text-gray-800 mb-3">Order Items</h2>{(order.items || []).map((item, index) => { const variantSummary = formatVariantSummary(item); return <div key={index} className="flex items-center gap-3 py-2 border-b border-gray-100 last:border-0">{item.product_image && <img src={item.product_image} alt="" className="w-12 h-12 rounded-lg object-cover" />}<div className="flex-1"><p className="text-sm font-medium text-gray-800">{item.product_name}</p><p className="text-xs text-gray-500">Qty: {item.quantity}</p>{variantSummary && <p className="text-xs text-blue-700 mt-0.5">{variantSummary}</p>}</div><p className="text-sm font-bold">â‚µ{(toNumber(item.price) * toNumber(item.quantity, 1)).toFixed(2)}</p></div>;})}<div className="mt-3 pt-3 border-t border-gray-100"><div className="flex justify-between text-xs text-gray-600 mb-1"><span>Total order value</span><span>â‚µ{grandTotal.toFixed(2)}</span></div><div className="flex justify-between text-xs text-gray-600 mb-1"><span>Initial payment</span><span>â‚µ{amountPaidNow.toFixed(2)}</span></div>{balanceDue > 0 && !remainingPaid && <div className="flex justify-between text-xs text-orange-700 mb-1"><span>Remaining balance via Order page</span><span>â‚µ{balanceDue.toFixed(2)}</span></div>}</div></Card>
        <Card className="p-5 bg-white mb-4"><h2 className="text-sm font-bold text-gray-800 mb-3">Delivery Information</h2><div className="space-y-1.5 text-xs text-gray-600"><p className="font-medium text-gray-800">{order.customer_name}</p><p>ðŸ“ {order.delivery_address}</p>{order.city && <p>{order.city}</p>}<p>ðŸ“ž {order.customer_phone}</p><p>âœ‰ï¸ {order.customer_email}</p>{hasEstDelivery && <p>ðŸ“… Est. delivery: {format(new Date(order.estimated_delivery), 'MMM d, yyyy')}</p>}</div></Card>
        <Link to={createPageUrl('Orders')}><Button variant="outline" className="w-full">â† Back to Orders</Button></Link>
      </div>
    </div>
  );
}
