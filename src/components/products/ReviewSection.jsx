    ...
    <div className="mt-8 bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4">
        ...
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
          {reviews.length > 0 && (
            <Button variant="ghost" size="sm" className="w-full sm:w-auto" onClick={() => setShowReviews(s => !s)}>
              {showReviews ? 'Hide Reviews' : 'Show Reviews'}
              {showReviews ? <ChevronUp className="ml-1 h-3 w-3" /> : <ChevronDown className="ml-1 h-3 w-3" />}
            </Button>
          )}
          <Button variant="outline" size="sm" className="w-full sm:w-auto" onClick={() => setShowForm(s => !s)}>
