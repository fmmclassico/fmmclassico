...
import {
  ...
  XCircle, Trash2, Check, AlertTriangle, Loader2
} from 'lucide-react';
...
// Cancel ONLY allowed in these statuses. Once "packed", cancel disappears.
...
export default function Orders() {
  ...
  useEffect(() => {
    ...
    if (status === 'cancelled') {
      // Payment cancelled: delete the pending order and notify user
      handlePaymentCancelled(orderNumber, user);
      setSearchParams({});
      return;
    }
    ...
    setIsVerifying(true);
    verifyPaymentWithRetry(orderNumber, user, 0);
  }, [user, searchParams]);

  const handlePaymentCancelled = async (orderNumber, currentUser) => {
    try {
      ...
      if (orders && orders.length > 0) {
        ...
        const productNames = order.items ? order.items.map(i => i.product_name).join(', ') : 'your item';

        // Delete the pending order so it does not show
        await base44.entities.Order.delete(order.id);

        // Send notification about failed/cancelled payment
        await base44.entities.Notification.create({
          user_email: currentUser.email,
          title: 'Payment Cancelled',
          message: 'Dear ' + firstName + ', your payment for "' + productNames + '" was cancelled. Your items are still in your cart. You can try again anytime from the checkout page.',
      ...
      console.error('[Orders] Cancel handling error:', err);
      toast.info('Payment was cancelled.');
    }
  };

  const verifyPaymentWithRetry = async (orderNumber, currentUser, attempt) => {
    ...
    try {
      ...
      if (orders && orders.length > 0) {
        ...
        // If callback already marked it as paid, done!
        if (order.payment_status === 'paid') {
          console.log('[Orders] Order already marked as paid by callback!');
          await clearCartAfterPayment(currentUser.email);
      ...
      } else if (result && result.data && result.data.paymentStatus) {
      ...
      const isFailed = hubtelStatus && (hubtelStatus.toLowerCase() === 'failed' || hubtelStatus.toLowerCase() === 'unpaid' || hubtelStatus.toLowerCase() === 'cancelled');

      if (isPaid) {
        // Update order from frontend as backup
        if (orders && orders.length > 0 && orders[0].payment_status !== 'paid') {
          ...
          // Send personalized success notification
          const firstName = getFirstName(order.customer_name);
          const productNames = order.items ? order.items.map(i => i.product_name).join(', ') : 'your order';
          await base44.entities.Notification.create({
            user_email: currentUser.email,
            title: 'Payment Confirmed!',
            message: 'Dear ' + firstName + ', your payment for "' + productNames + '" is confirmed. Thanks for buying from FMM CLASSICO! Kindly track your order status from your order page.',
      ...
      if (isFailed) {
        // Payment failed: delete order, send notification
        if (orders && orders.length > 0) {
          const order = orders[0];
          const firstName = getFirstName(order.customer_name);
          const productNames = order.items ? order.items.map(i => i.product_name).join(', ') : 'your item';
          ...
          await base44.entities.Notification.create({
            ...
            message: 'Dear ' + firstName + ', your payment for "' + productNames + '" failed. Kindly make sure to confirm payment so your order can be processed. Your items are still in your cart.',
            type: 'payment_pending',
            order_number: orderNumber,
            is_read: false,
            created_date: new Date().toISOString()
      ...
      if (attempt < MAX_ATTEMPTS - 1) {
        setTimeout(() => verifyPaymentWithRetry(orderNumber, currentUser, attempt + 1), DELAY_MS);
      } else {
        // Max attempts: final check
        const finalCheck = await base44.entities.Order.filter({ order_number: orderNumber });
        if (finalCheck && finalCheck.length > 0 && finalCheck[0].payment_status === 'paid') {
          await clearCartAfterPayment(currentUser.email);
          toast.success('Payment confirmed! Your order has been received.');
        } else {
  ...
  // ONLY show orders that are PAID. Pending/unpaid orders are NOT shown.
  ...
  const cancelOrderMutation = useMutation({
    mutationFn: async ({ order, reason }) => {
      ...
      // Send cancellation reason to admin notifications
      const adminEmails = (import.meta.env.VITE_ADMIN_EMAILS || '').split(',').map(e => e.trim()).filter(Boolean);
      for (const adminEmail of adminEmails) {
        await base44.entities.Notification.create({
          user_email: adminEmail,
          title: 'Customer Cancelled Order #' + order.order_number,
          message: 'Customer ' + order.customer_name + ' cancelled order #' + order.order_number + '. Reason: ' + (reason || 'No reason given') + '. Phone: ' + (order.customer_phone || 'N/A'),
          type: 'order_cancelled',
          order_id: order.id,
          order_number: order.order_number,
          is_read: false,
                ...
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="text-right">
                    ...
                    {order.payment_status === 'paid' && (
                      <Badge className="text-[10px] px-2 py-0.5 ml-1 bg-green-100 text-green-700">Paid</Badge>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <Link to={createPageUrl('OrderTracking') + '?id=' + order.id}>
                    <Button variant="outline" size="sm" className="rounded-lg text-xs">
                      <ChevronRight className="w-3 h-3 mr-1" /> Track Order
                    </Button>
                  </Link>
      ...
      {cancellingOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <Card className="max-w-sm w-full p-6 rounded-2xl shadow-xl bg-white">
            ...
            <div className="mb-4">
              <label className="text-sm font-medium text-gray-700">Reason for cancellation (required)</label>
              <textarea className="w-full mt-1 border rounded-xl p-2 text-sm" rows={3} value={cancelReason} onChange={(e) => setCancelReason(e.target.value)} placeholder="Please tell us why you want to cancel this order..." />
            </div>
            ...
            <div className="flex gap-2">
              ...
              <Button variant="destructive" className="flex-1 rounded-xl" onClick={() => cancelOrderMutation.mutate({ order: cancellingOrder, reason: cancelReason })} disabled={cancelOrderMutation.isPending || !cancelReason.trim()}>
