...
export default function Notifications() {
  ...
  var handleMarkOneRead = function(notif) {
    if (notif.is_read) return;
    supabase.from('notifications').update({ is_read: true }).eq('id', notif.id).then(refreshNotifs);
  };
  ...
  var handleMarkSelectedRead = function() {
    if (selectedNotifs.length === 0) return;
    setIsProcessing(true);
    supabase.from('notifications').update({ is_read: true }).in('id', selectedNotifs).then(function() {
      setSelectedNotifs([]);
      setIsProcessing(false);
      refreshNotifs();
    });
  ...
  var handleDeleteSelected = function() {
    if (selectedNotifs.length === 0) return;
    setIsProcessing(true);
    supabase.from('notifications').delete().in('id', selectedNotifs).then(function() {
      setSelectedNotifs([]);
      setIsProcessing(false);
      refreshNotifs();
    });
  ...
  var handleToggleSelect = function(notifId) {
    setSelectedNotifs(function(prev) {
      return prev.includes(notifId)
        ? prev.filter(function(id) { return id !== notifId; })
        : prev.concat([notifId]);
    });
  };
  ...
  var handleSelectAll = function() {
    if (selectedNotifs.length === notifications.length) {
      ...
      setSelectedNotifs(notifications.map(function(n) { return n.id; }));
    }
  };
  ...
  if (!user) {
    return (<div className="p-4"><p className="text-center text-gray-500">Loading...</p></div>);
  }
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      <div className="max-w-2xl mx-auto p-4">
        ...
        <div className="flex gap-2 mb-3 flex-wrap">
          {selectedNotifs.length > 0 && (
            <>
              <Button size="sm" variant="destructive" onClick={handleDeleteSelected} className="rounded-lg text-xs" disabled={isProcessing}>
                <Trash2 className="h-3 w-3 mr-1" />
                {isProcessing ? 'Deleting...' : 'Delete (' + selectedNotifs.length + ')'}
              </Button>
              <Button size="sm" variant="outline" onClick={handleMarkSelectedRead} className="rounded-lg text-xs" disabled={isProcessing}>
          ...
          {unreadCount > 0 && selectedNotifs.length === 0 && (
            <Button size="sm" variant="outline" onClick={handleMarkAllRead} className="rounded-lg text-xs" disabled={isProcessing}>
              <Check className="h-3 w-3 mr-1" />
        ...
        {notifications.length > 0 && (
          <div className="flex items-center gap-2 mb-3">
            <input type="checkbox" checked={selectedNotifs.length > 0} onChange={handleSelectAll} className="w-4 h-4 cursor-pointer" />
            <span className="text-xs text-gray-500">Select All ({selectedNotifs.length}/{notifications.length})</span>
          </div>
        )}

        {isLoading ? (
          <div className="space-y-3">
            {Array(4).fill(0).map(function(_, i) { return <Skeleton key={i} className="h-20 w-full rounded-xl" />; })}
          </div>
        ) : notifications.length === 0 ? (
          <div className="text-center py-12">
            ...
            <Link to={createPageUrl('Shop')}>
              <Button className="mt-4 bg-blue-800 text-white rounded-xl text-sm">Start Shopping</Button>
            </Link>
          </div>
        ) : (
          <div className="space-y-2">
            {notifications.map(function(notif) {
              var config = typeConfig[notif.type] || typeConfig.general;
              var Icon = config.icon;
              var isSelected = selectedNotifs.includes(notif.id);
              return (
                <div key={notif.id} className={"flex items-start gap-3 p-3 rounded-xl border " + (!notif.is_read ? 'bg-blue-50/50 border-blue-100' : 'bg-white border-gray-100') + (isSelected ? ' ring-2 ring-blue-300' : '')}>
                  <input type="checkbox" checked={isSelected} onChange={function() { handleToggleSelect(notif.id); }} className="w-4 h-4 mt-1 cursor-pointer flex-shrink-0" />
                  <div className="flex-1 cursor-pointer" onClick={function() { handleMarkOneRead(notif); }}>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {!notif.is_read && <div className="w-2 h-2 rounded-full bg-blue-500"></div>}
                    </div>
                  ...
                  {notif.order_number && (
                    <Link to={createPageUrl('Orders')} onClick={function(e) { e.stopPropagation(); }}>
