...
function paymentSummaryLabel(order) {
  ...
  if (order?.initial_payment_status === 'paid' || order?.payment_status === 'paid') {
    return order?.balance_payment_enabled ? `Balance payment enabled, â‚µ${getBalanceDue(order).toFixed(2)} pending` : `Initial payment received, â‚µ${getBalanceDue(order).toFixed(2)} left`;
  }
