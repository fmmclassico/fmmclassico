  ...
  const confirmRemainingBalanceMutation = useMutation({
    mutationFn: async (order) => {
      ...
      await appClient.entities.Order.update(order.id, {
        ...
        tracking_updates: (order.tracking_updates || []).concat([{ status: 'Balance Paid', message: `Admin confirmed the balance payment of â‚µ${getBalanceDue(order).toFixed(2)}.`, timestamp: now }]),
      });
  ...
  const markBalanceFailedMutation = useMutation({
    mutationFn: async (order) => {
      ...
      await appClient.entities.Order.update(order.id, {
        ...
        tracking_updates: (order.tracking_updates || []).concat([{ status: 'Balance Payment Failed', message: `Balance payment of â‚µ${getBalanceDue(order).toFixed(2)} was not completed successfully.`, timestamp: now }]),
      });
